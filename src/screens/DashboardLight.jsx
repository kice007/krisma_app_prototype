...
import { ArrowUpRight, ArrowDown, GraduationCap, Ellipsis, Bell, Eye } from 'lucide-react'
import StatusBar from '../components/StatusBar'
import TabBar from '../components/TabBar'
import gerard from '../assets/gerard.jpg'

const AVATAR = 'https://images.unsplash.com/photo-1659956956591-73af98b68fe8?w=200&q=80&fit=crop'
...
export default function DashboardLight() {
  ...
  return (
    ...
    >
      ...
      {/* Scrollable content */}
      <div style={{
        position: 'absolute', top: 62, left: 0, right: 0, bottom: 71,
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            >
              <Bell size={20} color="#0A0A0F" strokeWidth={1.5} />
            </div>
            <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <span style={{ fontFamily: '"Rethink Sans", sans-serif', fontSize: 42, fontWeight: 800, color: '#0A0A0F', lineHeight: 1.1 }}>
            EUR 12 765.00
          </span>
          <span style={{ fontFamily: 'Inter', fontSize: 12, color: '#9494B8' }}>
            ≈ 8 367 750 FCFA
          </span>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          {ACTIONS.map(({ Icon, label }) => (
            <div key={label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
              <div
                onClick={() => {
                  if (label === 'Envoyer') navigate('/envoyer-destinataire-light')
                  else if (label === 'Demande') navigate('/demande-destinataire-light')
                  else if (label === 'Scolarité') navigate('/scolarite-montant-light')
                }}
                style={{
                  width: 56, height: 56, borderRadius: 28, boxSizing: 'border-box',
                  backgroundColor: '#F0F0F8', border: '7px solid #4F46E5',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
                }}>
                <Icon size={20} color="#4F46E5" strokeWidth={2} />
              </div>
              <span style={{ fontFamily: 'Inter', fontSize: 11, fontWeight: 600, color: '#737373' }}>{label}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: '"Rethink Sans", sans-serif', fontSize: 16, fontWeight: 700, color: '#0A0A0F' }}>
              Transactions
            </span>
            <span style={{ fontFamily: 'Inter', fontSize: 13, fontWeight: 500, color: '#4F46E5', cursor: 'pointer' }}
          ...
          {/* Figma */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 16, borderRadius: 10, border: '0.5px solid #E5E5F0', backgroundColor: '#FFFF' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                <span style={{ fontFamily: '"Rethink Sans", sans-serif', fontSize: 14, fontWeight: 600, color: '#0A0A0F' }}>Figma</span>
                <span style={{ fontFamily: 'Inter', fontSize: 12, color: '#737373' }}>Aujourd'hui, 12:30</span>
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 3 }}>
              <span style={{ fontFamily: '"Rethink Sans", sans-serif', fontSize: 14, fontWeight: 700, color: '#0A0A0F' }}>− EUR 15</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 16, borderRadius: 10, border: '0.5px solid #E5E5F0', backgroundColor: '#FFFF' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 44, height: 44, borderRadius: 22, overflow: 'hidden', flexShrink: 0 }}>
                <img src={gerard} alt="De Alexandre" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                <span style={{ fontFamily: '"Rethink Sans", sans-serif', fontSize: 14, fontWeight: 600, color: '#0A0A0F' }}>De Alexandre</span>
                <span style={{ fontFamily: 'Inter', fontSize: 12, color: '#737373' }}>Hier, 08:00</span>
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 3 }}>
              <span style={{ fontFamily: '"Rethink Sans", sans-serif', fontSize: 14, fontWeight: 700, color: '#22C55E' }}>+ EUR 200</span>
              <span style={{ fontFamily: 'Inter', fontSize: 12, color: '#737373' }}>Dépôt</span>
            </div>
          </div>

          {/* Medium */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderRadius: 14, border: '0.5px solid #E5E5F0', backgroundColor: '#FFFF' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                <span style={{ fontFamily: 'Inter', fontSize: 14, fontWeight: 600, color: '#0A0A0F' }}>Medium</span>
                <span style={{ fontFamily: 'Inter', fontSize: 12, color: '#737373' }}>10 Mai, 18:00</span>
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 3 }}>
              <span style={{ fontFamily: '"Rethink Sans", sans-serif', fontSize: 14, fontWeight: 700, color: '#0A0A0F' }}>- EUR 99.00</span>
