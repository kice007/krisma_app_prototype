...
export default function ParentDashboardLight() {
    ...
    >
      ...
      <div style={{
        ...
        <motion.div {...fadeUp(0)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {/* Notif button */}
          <div style={{ position: 'relative' }} onClick={() => navigate('/parent-notifications-light')}>
            <div style={{
              width: 44, height: 44, borderRadius: 22,
              backgroundColor: '#EAEAF2', border: '1px solid #C8C8DC',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              cursor: 'pointer',
            }}>
              <Bell size={18} color="#1A1A2E" strokeWidth={1.5} />
            </div>
            <div style={{
        ...
        <motion.button
          {...fadeUp(0.06)}
          onClick={() => navigate('/parent-envoyer-beneficiaire-light')}
          whileTap={{ scale: 0.97 }}
          style={{
        ...
        <motion.div {...fadeUp(0.1)} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontFamily: '"Rethink Sans", sans-serif', fontSize: 16, fontWeight: 700, color: '#1A1A2E' }}>
              Mes bénéficiaires
            </span>
          ...
          <div style={{ display: 'flex', gap: 20 }}>
            {BENEFICIARIES.map(({ initials, color, name, country }) => (
              <div key={name} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                ...
                <span style={{ fontFamily: 'Inter', fontSize: 13, fontWeight: 600, color: '#1A1A2E', textAlign: 'center' }}>
                  {name}
                </span>
        ...
        <motion.div {...fadeUp(0.14)} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span
              onClick={() => navigate('/parent-historique-light')}
          ...
          {SENDS.map(({ initials, color, name, date, amount, badge, badgeColor, badgeBg }) => (
            <div
              ...
              style={{
                ...
                borderRadius: 12, border: '1px solid #DDDDE8',
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  <span style={{ fontFamily: 'Inter', fontSize: 14, fontWeight: 600, color: '#1A1A2E' }}>{name}</span>
                  <span style={{ fontFamily: 'Inter', fontSize: 12, color: '#737373' }}>{date}</span>
                </div>
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                <span style={{ fontFamily: '"Rethink Sans", sans-serif', fontSize: 14, fontWeight: 700, color: '#1A1A2E' }}>
                  {amount}
                </span>
      ...
      <ParentTabBar activeTab="Accueil" theme="light" />
    </motion.div>
  )
}
