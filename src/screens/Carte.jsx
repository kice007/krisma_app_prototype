  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-around' }}>
          {ACTIONS.map(({ Icon, label }) => (
            <div key={label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
              <div onClick={() => label === 'Paramètres' && navigate('/parametres-carte')} style={{
                width: 60, height: 60, borderRadius: 30, backgroundColor: '#4F46E5',
                display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
              }}>
                <Icon size={22} color="#FFFFFF" strokeWidth={1.5} />
              </div>
              <span style={{ fontFamily: 'Inter', fontSize: 12, fontWeight: 500, color: '#ffff' }}>{label}</span>
            </div>
          ))}
        </div>
        ...
        {/* IBAN Box */}
        <div style={{
          backgroundColor: '#0D1025', borderRadius: 14, padding: '16px 20px',
          border: '0.5px solid #1E1E2E',
          display: 'flex', flexDirection: 'column', gap: 8,
        }}>
          <span style={{ fontFamily: 'Inter', fontSize: 11, fontWeight: 600, color: '#555577', letterSpacing: 1 }}>
            IBAN
          </span>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: 'Inter', fontSize: 13, color: '#FFFFFF' }}>
              FR76 1468 9000 0100 4821 3300 075
            </span>
            <Copy size={18} color="#FFFFFF" strokeWidth={1.5} style={{ flexShrink: 0, marginLeft: 8, cursor: 'pointer' }} onClick={handleCopyIban} />
          </div>
        </div>

        {/* Share RIB */}
        <div style={{
          backgroundColor: '#0D1025', borderRadius: 14, height: 52,
          border: '0.5px solid #1E1E2E',
      ...
      <TabBar activeTab="Ma carte" theme="dark" />
