...
function ContactRow({ name, email, img, onClick }) {
  return (
    <div onClick={onClick} style={{
      ...
      padding: '14px 16px', border: '1px solid #161c2c',
      borderRadius: 12, cursor: 'pointer',
    }}>
      <img src={img} alt={name} style={{ width: 46, height: 46, borderRadius: 23, objectFit: 'cover', flexShrink: 0 }} />
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 3 }}>
        <span style={{ fontFamily: '"Rethink Sans", sans-serif', fontSize: 15, fontWeight: 600, color: '#FFFFFF', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {name}
        </span>
        <span style={{ fontFamily: 'Inter', fontSize: 13, color: '#737373' }}>{email}</span>
      </div>
    </div>
  )
}

export default function ParentEnvoyerBeneficiaire() {
  ...
  return (
    ...
    >
      <style>{`.peb-input::placeholder { color: #555577; }`}</style>
      <StatusBar />
      ...
      <div style={{
        ...
        <div onClick={() => navigate('/parent-dashboard')} style={{
        ...
        <div style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: '#1A1A3A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{
        ...
        backgroundColor: '#0A0A0F', borderRadius: '24px 24px 0 0',
        ...
        <div style={{ flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 20px 16px' }}>
            ...
            <div onClick={() => navigate('/parent-dashboard')} style={{
              width: 32, height: 32, borderRadius: 16, backgroundColor: '#2A2A4A',
              display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
            }}>
          ...
          <div style={{ padding: '0 20px 14px' }}>
            <div style={{
              height: 46, borderRadius: 14, backgroundColor: '#141830', border: '0.5px solid #2A2A4A',
              ...
              <input
                ...
                style={{ flex: 1, background: 'none', border: 'none', outline: 'none', fontFamily: 'Inter', fontSize: 14, color: '#FFFFFF' }}
              />
          ...
          {filteredRecents.length > 0 && (
            <div style={{ padding: '0 20px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              {filteredRecents.map(c => (
                <ContactRow key={c.name} {...c} onClick={() => navigate('/parent-envoyer-montant', { state: { recipient: c } })} />
              ))}
            </div>
          )}
